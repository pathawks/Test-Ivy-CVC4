...
pushd "$DIR"
# make clean
JAVA_CPPFLAGS=$JAVA_CPPFLAGS ./configure production --enable-language-bindings=java --with-antlr-dir=/space/hawks/CVC4/antlr-3.4
#Now just type make, followed by make check or make install, as you like.
# make
cp $JAR           $IVY"/libs/cvc4/cvc4-$VERSION.jar"
cp $LIBCVC4       $IVY"/libs/cvc4/libcvc4-$VERSION.so"
cp $LIBCVC4JNI    $IVY"/libs/cvc4/libcvc4jni-$VERSION.jar"
cp $LIBCVC4PARSER $IVY"/libs/cvc4/libcvc4parser-$VERSION.jar"
popd
